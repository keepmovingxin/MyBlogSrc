...
1. 在一个名为_ENV的upvalue作用域中编译chunk 
2. 将所有的自由名称转换成_ENV.var 
3. load或loadfile函数用全局环境初始化_ENV

也可以通过显式使用_EVN来访问被遮蔽的全局变量：
```
a = 13  
local a = 12  
print(a)      --> 12  
...
最后，提一下，lua_state启动后在注册表里`LUA_RIDX_GLOBALS`下标存放的全局表一定有一个元素是指向自己的，即_G.
